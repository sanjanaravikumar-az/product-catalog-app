...
async function testListProducts(): Promise<string | null> {
  ...
  products.slice(0, 5).forEach((p: any) => 
    console.log(`   - [${p.id.substring(0, 8)}...] ${p.engword} | ${p.price || 'N/A'} | Stock: ${p.stock || 0}`)
  );
...
async function testListComments(): Promise<string | null> {
  ...
  comments.slice(0, 5).forEach((c: any) => 
    console.log(`   - [${c.authorName}] "${c.content.substring(0, 50)}${c.content.length > 50 ? '...' : ''}"`)
  );
...
async function testCheckLowStock(): Promise<void> {
  ...
  if (data.lowStockProducts && data.lowStockProducts.length > 0) {
    ...
    data.lowStockProducts.forEach((p: any) => 
...
async function testUploadProductImage(productId?: string): Promise<string | null> {
  ...
  if (fs.existsSync(localImagePath)) {
    ...
    const testImageBase64 = '';
  ...
  const s3Key = productId 
    ? `products/${productId}_${Date.now()}-${fileName}`
    : `products/test_${Date.now()}-${fileName}`;
...
async function testProductFiltering(): Promise<void> {
  ...
  const searchFiltered = products.filter((p: any) => 
    p.engword.toLowerCase().includes(searchTerm)
  );
  ...
  // Test sorting
  const sortedByName = [...products].sort((a: any, b: any) => 
    a.engword.localeCompare(b.engword)
  );
  const sortedByPrice = [...products].sort((a: any, b: any) => 
    (a.price || 0) - (b.price || 0)
  );
  const sortedByStock = [...products].sort((a: any, b: any) => 
    (a.stock || 0) - (b.stock || 0)
  );
...
async function testLowStockReportGeneration(): Promise<void> {
  ...
  if (data?.lowStockProducts && data.lowStockProducts.length > 0) {
    ...
    const csvRows = data.lowStockProducts
...
async function runStorageTests(): Promise<void> {
  ...
  const { productId, imageKey: newImageKey } = await runTest('createProductWithImage', testCreateProductWithImage) || { productId: null, imageKey: null };
